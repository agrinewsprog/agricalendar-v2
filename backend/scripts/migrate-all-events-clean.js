...
rows.forEach((row, index) => {
  ...
  if (fields.length >= 28) {
    ...
    postgresSQL += `INSERT INTO events (id, name, image, start_date, end_date, start_time, end_time, color, location, description, state, region, tipo, website, slugevento, status, idioma, user_id, created_at, updated_at) VALUES (${id}, ${cleanName}, ${cleanImage}, ${convertDate(
      start_date
    )}, ${convertDate(end_date)}, ${start_time || "''"}, ${end_time || "''"}, ${
      color || "''"
    }, ${cleanLocation}, ${cleanDescription}, ${cleanState}, ${cleanRegion}, ${cleanTipo}, ${cleanWebsite}, ${cleanSlug}, 'PUBLISHED', ${
      idioma || 1
