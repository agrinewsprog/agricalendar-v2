...
`;
  ...
  matches.forEach(insertStatement => {
    ...
    rows.forEach((row, index) => {
      ...
      if (fields.length >= 28) { // Mínimo de campos esperados
        const [
          id, name, image, start_date, end_date, start_time, end_time, 
          color, location, description, state, region, tipo, website, 
          id_parent, idioma, slugevento, seo_title, seo_desc, views, 
          visitante, programa, ponentes, registro, id_user, descuento, 
          id_medio, url_redirect
        ] = fields;
  ...
  ${id}, ${name}, ${image}, ${convertDate(start_date)}, ${convertDate(end_date)}, 
...
const outputFile = 'C:\\Users\\Samuel\\Desktop\\proyectos_github\\agricalendar-v2\\backend\\scripts\\events-postgres.sql';
